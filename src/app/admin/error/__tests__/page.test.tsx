...
describe('AuthErrorPage', () => {
  ...
  it('renders access denied error message', () => {
    // Mock error code
    (useSearchParams as jest.Mock).mockReturnValue({
      get: jest.fn().mockReturnValue('AccessDenied'),
    });
  ...
  it('renders configuration error message', () => {
    ...
    expect(screen.getByText('There is a problem with the authentication configuration.')).toBeInTheDocument();
  });
  
  it('renders email sign-in error message', () => {
    // Mock error code
    (useSearchParams as jest.Mock).mockReturnValue({
      get: jest.fn().mockReturnValue('EmailSignin'),
    });
    
    render(<AuthErrorPage />);
    
    expect(screen.getByText('Email Sign-in Error')).toBeInTheDocument();
    expect(screen.getByText('The email could not be sent. Please try again later.')).toBeInTheDocument();
  });
});
